...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const hardcodedAccessKey = "igiJIIQhPtMIhwwehifzVfhnDWcG3l8XMjYyjNbiaz0" // Your Unsplash API key
    const accessKey = envAccessKey || hardcodedAccessKey
    ...
    if (accessKey && accessKey.length > 10) {
      ...
      if (response.ok) {
        ...
        console.log("[v0] Unsplash API response:", data.results.length, "results")
